#!/usr/bin/env node
// 유료 Apple Developer 계정 없이(Personal Team 무료 서명) 실기기에 로컬로 설치하기 위한
// 임시 빌드 스크립트. Sign In with Apple / 푸시 알림 capability는 Personal Team에서 지원되지
// 않아서, 이 두 네이티브 모듈만 빌드 동안 잠깐 제외했다가 끝나면 원상복구한다.
// Release 설정으로 빌드해서 JS 번들을 앱 안에 내장 — Metro/같은 네트워크 없이도 독립 실행된다.
...
const result = spawnSync(
  "npx",
  ["expo", "run:ios", "--device", deviceArg, "--configuration", "Release"],
